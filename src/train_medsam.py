...
from monai.networks.nets import UNet  # (kept if you import elsewhere; not used here)
...
class BKMedSAMExperiment:
    ...
    def setup(self):
        ...
        self.epoch = 0 if self.state is None else int(self.state.get("epoch", 0))
        self.best_score = 0.0 if self.state is None else float(self.state.get("best_score", 0.0))
    ...
    def setup_model(self):
        ...
        # torch.compile can fail on some envs; guard it
        try:
    ...
    def _prep_batch(self, batch):
        ...
        bmode = torch.cat([bmode, bmode, bmode], dim=1) / (bmode.max() + 1e-8)
        ...
        return bmode, needle_mask, prostate_mask, ood_mask, label, involvement, core_ids, patient_ids
    ...
    def run_train_epoch(self, loader, desc="train"):
        ...
        for train_iter, batch in enumerate(tqdm(loader, desc=desc)):
            ...
            bmode, needle_mask, prostate_mask, ood_mask, label, involvement, core_ids, patient_ids = self._prep_batch(batch)
            ...
            with torch.cuda.amp.autocast(enabled=self.config.use_amp):
                ...
                mean_predictions_in_needle = torch.stack(
                    [predictions[batch_idx == j].sigmoid().mean() for j in range(len(bmode))]
                )

                prostate_masks = prostate_mask > 0.5
                predictions_p, batch_idx_p = MaskedPredictionModule()(heatmap_logits, prostate_masks)
                mean_predictions_in_prostate = torch.stack(
                    [predictions_p[batch_idx_p == j].sigmoid().mean() for j in range(len(bmode))]
                )
    ...
    def run_eval_epoch(self, loader, desc="eval"):
        ...
        for train_iter, batch in enumerate(tqdm(loader, desc=desc)):
            ...
            if train_iter % 5 == 0 and self.config.wandb.log_images:
                wandb_im = self.show_example(batch)  # pass raw batch
                if wandb_im is not None:
    ...
    def create_and_report_metrics(self, results_table, desc="eval"):
        ...
        if keep.sum() > 0:
            ...
            metrics_hi = calculate_metrics(core_probs, core_labels, log_images=self.config.wandb.log_images)
    ...
    def show_example(self, batch):
        ...
        nz = masked_pred[masked_pred > 0]
        ax[2].imshow(pred[0, 0], **kwargs)
        ax[2].set_title(f"Predicted label: {float(nz.mean().item()) if nz.numel() else 0.0:.2f}")
